...
# FIX: Add CATBOOST_OUTPUT_DIR to imports
from config import (
    PROCESSED_DIR, 
    LEADERBOARD_DIR,
    TEST_RESULTS_DIR, 
    WANDB_OUTPUT_DIR,
    CATBOOST_OUTPUT_DIR  # ← Add this
)
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=TEST_RESULTS_DIR / "current_modeling",
    ...
    best_model_name = min(
        [
            ("xgboost_poisson", metrics["xgboost_poisson"]["mae"]),
            ("lightgbm_poisson", metrics["lightgbm_poisson"]["mae"]),
            ("catboost_poisson", metrics["catboost_poisson"]["mae"]) if HAS_CATBOOST else ("catboost_poisson", 9999),
            ("zip", metrics["zip"]["mae"]),
            ("prophet_hourly_total", metrics["prophet_hourly_total"]["mae"]) if HAS_PROPHET else ("prophet_hourly_total", 9999),
        ],
        key=lambda x: x[1]
    )[0]
