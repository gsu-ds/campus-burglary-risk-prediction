...
# CRITICAL FIX: Ensure 'rows' and 'cols' are integers before applying the comma format specifier
def log_step(step_name: str, df: pd.DataFrame) -> None:
    ...
    console.print(f"[green]✓ {step_name}[/green] [cyan]→ shape: {rows:,} x {cols}[/cyan]")


def show_pipeline_table() -> None:
    """Display a Rich table summarizing all pipeline steps. FIXES ValueError: Cannot specify ',' with 's'"""
    if not pipeline_log:
    ...
    for entry in pipeline_log:
        # Safely format the row and column counts
        rows_str = f"{entry['rows']:,}" if isinstance(entry['rows'], int) else entry['rows']
        cols_str = str(entry['cols']) if isinstance(entry['cols'], int) else entry['cols']
...
def attach_npu(gdf: gpd.GeoDataFrame, npu_shp: Path) -> gpd.GeoDataFrame:
    """Attach NPU labels using nearest polygon (handles boundary fuzz)."""
...
def impute_missing_zone_from_npu(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["zone_int_cleaned"] = df["zone_int"].fillna(df["npu_label"].map(mapping))
...
def engineer_date_context_features(
    ...
    """Create core temporal + contextual features. Incorporates 4-hour binning."""
    df = df.copy()
    console.print("\n[bold cyan]═══ Engineering date and contextual features ═══[/bold cyan]\n")
    ...
    # 4-hour block logic matching requested visuals: 0-6, 6-12, 12-18, 18-24
    bins = [0, 6, 12, 18, 25]
    labels = [
        "Night (0-6)",
        "Morning (6-12)",
    ...
    # Set labels based on the hours column created in the prior version to maintain consistency
    df['hour'] = df['incident_datetime'].dt.hour
    df["time_bin"] = pd.cut(df["hour"], bins=bins, labels=labels, right=False, include_lowest=True)


    # Previous 4-hour block (for Rich Summary only, using original names for simplicity)
    bins_rich = [0, 5, 9, 13, 17, 21, 25]
    labels_rich = [
        "Early Night (0–4)",
        "Early Morning (5–8)",
        "Late Morning (9–12)",
    ...
    df["hour_block_rich"] = pd.cut(df["incident_hour"], bins=bins_rich, labels=labels_rich, right=False)
    ...
    log_step("Step 8: Date and context features (including 4-bin time_bin)", df)
    return df
...
def merge_weather_data_basic(df: pd.DataFrame) -> pd.DataFrame:
    ...
    console.print("[green]✓ Initial hourly + daily weather merged (2024 subset).[/green]")
    return df
