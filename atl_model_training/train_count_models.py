...
def main():
    ...
    if (wandb_project in [None, "", "prompt"]) and HAS_WANDB:
        wandb_project = input("Enter wandb project name (blank to skip): ").strip() or None
    if (wandb_run_name in [None, "", "prompt"]) and HAS_WANDB and wandb_project:
        wandb_run_name = input(
            "Enter wandb run name (blank for auto-generated): "
        ).strip() or None
    ...
    if wandb_project and HAS_WANDB:
        ...
        print("wandb project not provided; skipping wandb logging.")
    else:
