...
Run multiple models on multiple datasets using simple train/test split and 
Rolling-origin Cross-Validation (CV).
"""
...
WANDB_ENTITY = "YOUR_WANDB_USERNAME_OR_ENTITY" # <-- REQUIRED: Update this to your entity!
# --------------------------------
...
def run_rolling_cv(
    ...
    for fold_idx, (train_end, test_end) in enumerate(folds, start=1):
        ...
        if hasattr(model, "fit"):
            ...
            # Fallback for baselines if needed
            preds = model.predict(X_test)
...
def run_for_dataset(
    ...
    for model_name, model in models.items():
        ...
        # optional W&B
        if HAS_WANDB:
            # W&B runs are created for each (Dataset + Model + CV) combination
            run = wandb.init(
                project=WANDB_PROJECT,
                entity=WANDB_ENTITY,
                name=f"{dataset_name}_{model_name}_rolling_cv",
                config={"dataset": dataset_name, "model": model_name},
                reinit=True, # Allows re-running init within a process
            )
            wandb.log(row)
            run.finish()
...
def main() -> None:
    console.print(
        Panel(
            "[bold cyan]Multi-Dataset Modeling and Cross-Validation[/bold cyan]",
            border_style="cyan",
        )
    )
